    ...
    public class CreateStudentHandler : IRequestHandler<CreateStudentCommand, bool>
    {
        ...
        public async Task<bool> Handle(CreateStudentCommand request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation(
                "User {UserId} attempting to create student {Email} in Department {DepartmentId}",
                adminId, request.Email, request.DepartmentId);

            // 🔐 Authorization
            if (!_currentUserService.IsAdmin)
            {
                _logger.LogWarning("Unauthorized student creation attempt by User {UserId}", adminId);
                throw new UnauthorizedException("Only admin can create student");
            }
            ...
            {
                _logger.LogWarning(
                    "Duplicate admission number attempt {AdmissionNumber} by User {UserId}",
            ...
            if (string.IsNullOrWhiteSpace(request.Email))
            {
                _logger.LogWarning("Student creation failed: Email missing (User {UserId})", adminId);
                throw new BuisnessRuleException("Email is required");
            }
            ...
            {
                _logger.LogError("Failed to create identity user for student {Email}", request.Email);
            ...
            if (result > 0)
            {
                // ✅ Audit log
                _logger.LogInformation(
                    "User {UserId} created student {AdmissionNumber} ({Email}) successfully",
                    adminId, admissionNumber, request.Email);
            ...
            {
                _logger.LogError("Failed to create student record for {Email}", request.Email);
            }
